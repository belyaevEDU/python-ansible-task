...
Файлы раздела 2 находятся в корневой папке, файлы - [Dockerfile](/Dockerfile). *В задании указано, что в репозитории должно содержаться 3 директории, но docker build не позволяет копировать в образ файлы из родительских папок.*
...
К тому же, в задании прописано выполнять HTTP запросы к сервису [httpstat.us](https://httpstat.us), но сервис в оффлайне [из-за огромного кол-ва трафика](https://github.com/aaronpowell/httpstatus/issues/165#issuecomment-3043685050). Поэтому, я использовал [зеркало](https://tools-httpstatus.pickup-services.com/).
